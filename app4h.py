...
def get_data(ticker, retries=5, delay=10, timeout=30):
    ...
    for i in range(retries):
        try:
            return yf.download(ticker, period="6mo", interval="4h", progress=False, timeout=timeout)
...
if __name__ == "__main__":
    ...
    md_content += "This report identifies stocks from the S&P 500 that have experienced a SuperTrend trend change on the most recent 4-hour candle. The calculation is based on the Pine Script logic from the project's README, using a 10-period ATR with a multiplier of 3.\n\n---\n"
    ...
    if flipped_tickers:
        ...
        md_content += "**No tickers experienced a SuperTrend trend change on the most recent 4-hour candle.**\n"

    with open("summary4h.md", "w") as f:
        f.write(md_content)

    print("Summary saved to summary4h.md")
